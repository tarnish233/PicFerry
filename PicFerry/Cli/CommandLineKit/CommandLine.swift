...
public class CommandLineKit {
  ...
  public enum ParseError: Error, CustomStringConvertible {
    ...
    case invalidValueForOption(String, [String])

    /** Thrown if an Option with required: true is missing */
    case missingRequiredOptions([String])

    public var description: String {
      switch self {
      case let .invalidArgument(arg):
        return "Invalid argument: \(arg)"
      case let .invalidValueForOption(optionDescription, vals):
        let vs = vals.joined(separator: ", ")
        return "Invalid value(s) for option \(optionDescription): \(vs)"
      case let .missingRequiredOptions(optionDescriptions):
        return "Missing required options: \(optionDescriptions)"
      }
    }
  }
  ...
  public func parse(strict: Bool = false) throws {
    ...
    for (idx, arg) in argumentsEnumerator {
      ...
      for option in _options where option.flagMatch(flag) {
        ...
        guard option.setValue(vals) else {
          throw ParseError.invalidValueForOption(option.flagDescription, vals)
        }
    ...
    guard missingOptions.count == 0 else {
      throw ParseError.missingRequiredOptions(missingOptions.map(\.flagDescription))
    }
