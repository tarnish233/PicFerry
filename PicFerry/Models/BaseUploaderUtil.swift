...
final class BaseUploaderUtil {
    ...
    public static func compressPng(_ data: Data, factor: Int = 100) -> Data {
        guard factor > 0, factor < 100, !data.isEmpty else {
            return data
        }

        guard let source = malloc(data.count)?.assumingMemoryBound(to: UInt8.self) else {
            return data
        }
        data.copyBytes(to: source, count: data.count)

        var compressed: UnsafeMutablePointer<UInt8>?
        let compressedSize = Int(_data2Data(&compressed, Int32(factor), source, data.count))

        // libminipng takes ownership of `source` only when compression succeeds.
        // Freeing it again on the success path causes a deterministic double-free.
    ...
    static func _getRrandomFileName(_ fileExtension: String?) -> String {
        ...
        guard let fileExtension, !fileExtension.isEmpty else { return random }
    ...
    static func parseSaveKeyPath(_ saveKeyPath: String?, _ filenameComponent: String) -> String {
        let keyPath = saveKeyPath.flatMap { $0.isEmpty ? nil : $0 } ?? _defaultSaveKeyPath
        return _parseVariables(keyPath, filenameComponent, otherVariables: nil)
    }

    nonisolated static func makePublicURL(domain: String, path: String) -> String {
        let normalizedDomain = domain.hasSuffix("/") ? String(domain.dropLast()) : domain
        let normalizedPath = path.hasPrefix("/") ? String(path.dropFirst()) : path
        return "\(normalizedDomain)/\(normalizedPath)".urlOutputEncoded()
    }
    ...
    static func getSaveConfiguration(_ fileUrl: URL?, _ fileData: Data?, _ saveKeyPath: String?) -> [String: Any]? {
        ...
        if let fileUrl = fileUrl {
            ...
            if fileName.pathExtension.isEmpty {
                fileName = fileName.appendingPathExtension(ext: "jpg") ?? "\(fileName).jpg"
            }
    ...
    static func getSaveConfigurationWithB64(_ fileUrl: URL?, _ fileData: Data?, _ saveKeyPath: String?) -> [String: Any]? {
        ...
        guard let fileBase64 else { return nil }
        ...
        return ["retData": retData as Any, "fileBase64": fileBase64, "fileName": fileName, "mimeType": mimeType, "saveKey": saveKey]
    }
