...
@MainActor
final class URLSchemeExt {
    static let shared = URLSchemeExt()
    
    func handleURL(_ urlStr: String) {
        Logger.shared.verbose("开始解析 URLScheme 参数: \(urlStr)")

        guard let url = URL(string: urlStr), let scheme = url.scheme else {
            Logger.shared.error("URLScheme 参数解析失败")
            return
        }
        
        let prefix = "\(scheme)://"
        guard urlStr.hasPrefix(prefix) else {
            Logger.shared.warn("URLScheme 参数缺少 // 分隔符")
            return
        }
        let param = String(urlStr.dropFirst(prefix.count))
